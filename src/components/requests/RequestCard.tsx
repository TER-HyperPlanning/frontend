...
export default function RequestCard({
  ...
  return (
    <div className={`bg-[#F4F6F8] rounded-2xl border-l-4 ${borderColor} p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1`}>
      <div className="flex flex-col md:flex-row justify-between gap-6">
        <div className="flex flex-col gap-3 flex-1">
          <div className={`flex items-center gap-2 text-xs font-semibold px-3 py-1 rounded-full w-fit ${statusStyle}`}>
            {statusIcon} {status}
          </div>
          <h3 className="text-xl font-bold text-[#003A68]">{teacher}</h3>
          <p className="text-gray-800 font-medium">{subject}</p>
          <p className="text-gray-500 text-sm">{formation}</p>
        ...
        <div className="flex flex-col items-start md:items-center gap-2">
          ...
          <button
            ...
            className="mt-2 text-white bg-[#003A68] hover:bg-[#00509E] px-4 py-2 rounded-xl text-sm"
          >
            Voir détails
          </button>
        </div>
